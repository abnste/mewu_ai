...
public class OpenAiCompatibleProvider : IAiProvider
{
    ...
    public virtual async Task<AiResult> SendAsync(AiRequest request,CancellationToken token)
    {
        ...
        var streaming=request.StreamingProgress is not null&&Capabilities.SupportsStreaming;var body=JsonSerializer.Serialize(new{model=_settings.Model,messages,temperature=.2,stream=streaming});using var req=Create(HttpMethod.Post,"chat/completions");req.Content=new StringContent(body,Encoding.UTF8,"application/json");using var res=await Client.SendAsync(req,HttpCompletionOption.ResponseHeadersRead,token);if(!res.IsSuccessStatusCode)throw new InvalidOperationException($"AI 请求失败（HTTP {(int)res.StatusCode}）");if(streaming){using var responseStream=await res.Content.ReadAsStreamAsync(token);using var reader=new StreamReader(responseStream);var answer=new StringBuilder();while(await reader.ReadLineAsync(token) is { } line){if(!StreamingResponseParser.TryParse(line,out var delta,out var done))continue;if(done)break;if(delta.Length>0){answer.Append(delta);request.StreamingProgress!.Report(delta);}}return StructuredResponseParser.Parse(answer.ToString());}
        var json=await res.Content.ReadAsStringAsync(token);using var doc=JsonDocument.Parse(json);var answerText=doc.RootElement.GetProperty("choices")[0].GetProperty("message").GetProperty("content").GetString()??string.Empty;return StructuredResponseParser.Parse(answerText);
    }
    ...
    private void Validate(AiRequest r){foreach(var a in r.Attachments){if(a.Type==AiAttachmentType.Image&&!Capabilities.SupportsImage)throw new NotSupportedException("当前模型不支持图片");if(a.Type==AiAttachmentType.Video&&!Capabilities.SupportsVideo)throw new NotSupportedException("当前模型不支持视频");if(Capabilities.AcceptedMimeTypes.Count>0&&!Capabilities.AcceptedMimeTypes.Contains(a.MimeType))throw new NotSupportedException($"当前模型不接受 {a.MimeType} 附件");var size=a.Data?.LongLength??(a.FilePath is null?0:new FileInfo(a.FilePath).Length);if(size>Capabilities.MaxAttachmentSize)throw new InvalidOperationException("附件超过当前模型限制");if(a.Duration is { } duration&&Capabilities.MaxVideoDuration>TimeSpan.Zero&&duration>Capabilities.MaxVideoDuration)throw new InvalidOperationException("视频时长超过当前模型限制");}}
}
