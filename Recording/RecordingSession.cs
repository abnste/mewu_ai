...
public sealed class RecordingSession : IDisposable
{
    ...
    public void Start()
    {
        ...
        var global=new Rectangle(_region.X,_region.Y,_region.Width,_region.Height);var sources=new List<RecordingSourceBase>();foreach(var screen in Forms.Screen.AllScreens){var part=Rectangle.Intersect(global,screen.Bounds);if(part.IsEmpty)continue;sources.Add(new DisplayRecordingSource(screen.DeviceName){SourceRect=new ScreenRecorderLib.ScreenRect(part.X-screen.Bounds.Left,part.Y-screen.Bounds.Top,part.Width,part.Height),Position=new ScreenPoint(part.X-_region.X,part.Y-_region.Y),OutputSize=new ScreenSize(part.Width,part.Height),IsCursorCaptureEnabled=_settings.IncludeRecordingCursor,IsBorderRequired=false});}if(sources.Count==0)throw new InvalidOperationException("选区不在可录制显示器范围内");
        var width=_region.Width-( _region.Width%2);var height=_region.Height-(_region.Height%2);var options=new RecorderOptions{SourceOptions=new SourceOptions{RecordingSources=sources},OutputOptions=new OutputOptions{RecorderMode=RecorderMode.Video,OutputFrameSize=new ScreenSize(width,height)},VideoEncoderOptions=new VideoEncoderOptions{Encoder=new H264VideoEncoder(),Framerate=_settings.RecordingFps,Quality=75,IsHardwareEncodingEnabled=true,IsFixedFramerate=true},AudioOptions=new AudioOptions{IsAudioEnabled=false},MouseOptions=new MouseOptions{IsMousePointerEnabled=_settings.IncludeRecordingCursor}};
