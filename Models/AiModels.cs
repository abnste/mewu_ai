...
public sealed record AiAttachment(AiAttachmentType Type,string MimeType,byte[]? Data=null,string? FilePath=null);
public sealed record AiMessage(string Role,string Text);
public sealed class AiRequest { public string Prompt { get; init; }=string.Empty; public List<AiMessage> History { get; init; }=[]; public List<AiAttachment> Attachments { get; init; }=[]; }
...
public sealed record AiAnnotation(double X,double Y,double Width,double Height,string Text,string Type);
