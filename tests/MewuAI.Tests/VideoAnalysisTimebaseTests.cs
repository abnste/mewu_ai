...
public sealed class VideoAnalysisTimebaseTests
{
    ...
    [Fact]
    public async Task CalibratedVideoKeepsEventsDurationAndOriginalBytes()
    {
        ...
        try
        {
            ...
            composition.Clips.Add(MediaClip.CreateFromColor(Windows.UI.Color.FromArgb(255,0,255,0),TimeSpan.FromSeconds(2)));
            ...
            {
                preparedPath=prepared.Path;Assert.InRange(prepared.Duration.TotalSeconds,5.9,6.1);
                ...
                foreach(var (seconds,channel) in new[]{(.5,2),(2.5,0),(4.5,1)})
                {
