    ...
    internal static void Run(Application app,AppHost host,CaptureOverlayWindow overlay,bool english)
    {
        ...
        overlay.Loaded+=(_,_)=>app.Dispatcher.BeginInvoke(DispatcherPriority.ApplicationIdle,new Action(async()=>
        {
            try
            {
                ...
                var corrections=new[]
                {
                    new{Question="1",Observed="= (x⁴y⁻⁵)³ / (xy²)\n= x¹²y⁻¹⁵ / (xy²)\n= x^(12−1)y^(−15−2)\n= x¹³y⁻¹⁷\n= x¹³/y¹⁷",Expected="x¹¹/y¹⁷",Reason=L("12−1 应为 11，原卷后两行写成了 13。\n正确结果为 x¹¹/y¹⁷。","12−1 is 11; the final two lines use 13.\nThe correct result is x¹¹/y¹⁷."),Box=new Rect(.18,.18,.30,.25)},
                ...
                var path=Path.GetFullPath("docs/images/hkdse-2025-reviewed-steps-"+(english?"en":"zh")+".png");Directory.CreateDirectory(Path.GetDirectoryName(path)!);
