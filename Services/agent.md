...
- WorkBuddy 设置页“测试连接”只做 ACP 握手、会话、模型和思考选项协商，不发送对话、不消耗额度，独立使用 30 秒有界超时；不要复用完整文字轮次的 90 秒等待，否则 ACP 卡住时用户会误以为应用死循环。
...
- 2026-09-06 对话渠道按实际配置可用：API 列表中的每个有效接入点、Hermes、Codex、WorkBuddy、MiniMax Code 可同时保留配置，不再依赖单选启用开关。覆盖层仅在存在多个可用渠道时显示渠道选择器，并持久化上一次选择作为默认；历史按渠道与模型隔离，切换渠道不得回退到默认 Provider。
