...
internal static partial class QqMailContextService
{
    ...
    internal static async Task<string> TryBuildAsync(string prompt,string? screenText,Models.AppSettings settings,CancellationToken cancellationToken)
    {
        try
        {
            ...
            var token=settings.QqMailMcpEnabled?await QqMailMcpService.ResolveTokenAsync(timeout.Token).ConfigureAwait(false):null;
            if(token is null)
            {
                // QQ 未授权：优先尝试网易邮箱网页会话（扫码授权）拉取收件箱实时上下文。
                var netEase=settings.NetEaseMailEnabled?await TryBuildNetEaseAsync(addresses,NetEaseMailService.IsConfigured(settings),timeout.Token).ConfigureAwait(false):null;
    ...
    private static string FormatNetEase(string account,JsonArray messages,string[] addresses,bool canSend)
    {
