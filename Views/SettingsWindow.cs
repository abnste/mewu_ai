...
{
    private readonly AppHost _host; private readonly ComboBox _delay=new(),_providerType=new(); private readonly TextBox _baseUrl=new(),_model=new(); private readonly PasswordBox _apiKey=new(); private readonly CheckBox _history=new(),_voice=new(),_autoVoice=new();
    ...
    private UIElement Ai(){var p=Panel();p.Children.Add(Text("API Key 使用 Windows DPAPI 加密，仅保存在本机当前用户目录。"));_providerType.Items.Add("OpenAICompatible");_providerType.Items.Add("MiniMax");p.Children.Add(_providerType);p.Children.Add(Label("Base URL",_baseUrl));p.Children.Add(Label("Model",_model));var keyPanel=Panel();keyPanel.Children.Add(Text("API Key（留空则保留现有密钥）"));_apiKey.Margin=new Thickness(0,4,0,12);_apiKey.Padding=new Thickness(8);keyPanel.Children.Add(_apiKey);p.Children.Add(keyPanel);var provider=_host.Settings.Providers.FirstOrDefault();_providerType.SelectedIndex=provider?.Type=="MiniMax"?1:0;_baseUrl.Text=provider?.BaseUrl??"https://api.openai.com/v1";_model.Text=provider?.Model??"gpt-4.1-mini";return p;}
