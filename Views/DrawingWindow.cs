...
public sealed class DrawingWindow : Window
{
    ...
    public DrawingWindow(BitmapSource source)
    {
        ...
        _ink.Background=new ImageBrush(source){Stretch=Stretch.Uniform};_ink.DefaultDrawingAttributes=new DrawingAttributes{Color=Colors.Red,Width=4,Height=4,FitToCurve=true};_ink.StrokeCollected+=(_,_)=>_redo.Clear();
        var bar=new StackPanel{Orientation=Orientation.Horizontal,Margin=new Thickness(10)};Btn(bar,"红",()=>_ink.DefaultDrawingAttributes.Color=Colors.Red);Btn(bar,"蓝",()=>_ink.DefaultDrawingAttributes.Color=Color.FromRgb(49,140,255));Btn(bar,"细",()=>_ink.DefaultDrawingAttributes.Width=_ink.DefaultDrawingAttributes.Height=3);Btn(bar,"粗",()=>_ink.DefaultDrawingAttributes.Width=_ink.DefaultDrawingAttributes.Height=9);Btn(bar,"撤销",Undo);Btn(bar,"重做",Redo);Btn(bar,"清空",()=>_ink.Strokes.Clear());Btn(bar,"复制",()=>Clipboard.SetImage(Render()));Btn(bar,"保存",Save);Btn(bar,"贴图",()=>new PinnedImageWindow(Render()).Show());
        var grid=new Grid();grid.RowDefinitions.Add(new RowDefinition{Height=GridLength.Auto});grid.RowDefinitions.Add(new RowDefinition());Grid.SetRow(_ink,1);grid.Children.Add(bar);grid.Children.Add(_ink);Content=grid;
