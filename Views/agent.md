...
- 首轮视频回答没有有效时间轴批注时，应在同一请求所有权和附件租约内做至多一次受控补标请求；成功批注需在 Markdown 回答末尾生成本地可点击“跳到/播放”链接，链接直接驱动原位视频 seek 或区间跟踪，不得伪装成外部 URL、打开浏览器或新窗口。撤销重做恢复回答时也要按快照批注重建链接。
- 有效视频批注落地后按全部关键帧的最早时间自动跳转、暂停并渲染第一处标记；多个固定帧分别生成跳转链接，动作区间同时生成“播放区间”链接和每个关键帧的独立跳转链接。跳转链接只 seek 后暂停，播放链接才启动区间跟踪。
